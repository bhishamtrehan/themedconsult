...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login($login, $password, $remember, $login_by_username, $login_role ,$login_by_email)
	{
		...
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login, $login_role))) {	// login ok
	...
	{
		return ((strlen($username) > 0) AND $this->ci->users->is_username_available($username,$role));
	}
	...
	{
		return ((strlen($email) > 0) AND $this->ci->users->is_email_available($email,$role));
	}
	...
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_username($login))) {
