<?php 
    ...
    class Acl
    {
		...
		private $perm_data			            = 'mc_perm_data';			// Perm data
		private $role_data	                    = 'mc_roles';			    // Role data
		private $role_perms                 	= 'mc_role_perms';     	// Role perms 
		private $users           	            = 'mc_users';	            // User details 
		private $user_perms           	        = 'mc_user_perms';	        // User details 
		private $user_roles	                    = 'mc_user_roles';	        // User roles
		
        function __construct($config=array()) {
            $this->ci = &get_instance();
            $this->perm_data   = $this->perm_data;
            $this->role_data   = $this->role_data;
            $this->role_perms  = $this->role_perms;
            $this->users_table = $this->users;
            $this->user_perms  = $this->user_perms;
        ...
        function getPermKeyFromID($permID) {
            ...
            $sql = $this->ci->db->get($this->perm_data,1);
            $data = $sql->result();
        ...
        function getRoleNameFromID($roleID) {
            ...
            $sql = $this->ci->db->get($this->role_data);
            $data = $sql->result();
        ...
        function getUserRoles() {
            ...
            $sql = $this->ci->db->get($this->user_roles);
            $data = $sql->result();
        ...
        function getAllPerms($format='ids') {
            ...
            $sql = $this->ci->db->get($this->perm_data);
            $data = $sql->result();
        ...
        function getRolePerms($role) {
            ...
            $sql = $this->ci->db->get($this->role_perms); //$this->db->select($roleSQL);
            $data = $sql->result();
            $perms = array();
            foreach( $data as $row )
            {
            ...
            $this->ci->db->order_by('addDate','asc');
            $sql = $this->ci->db->get($this->user_perms);
            $data = $sql->result();
