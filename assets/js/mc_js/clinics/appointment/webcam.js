...
// Everything is under a 'webcam' Namespace
 var baseUrl2 = document.location.origin+'/themedconsult/assets/images/';
 var baseUrl4 = document.location.origin+'/themedconsult/assets/js/webcam';
window.webcam = {
	version: '1.0.9',
	...
	swf_url: baseUrl4+'/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	shutter_url: baseUrl4+'/shutter.mp3', // URI to shutter.mp3 sound
		...
		switch (type) {
			...
			case 'error':
				...
				if (!this.fire_hook('onError', msg)) {
					$("#wcMain").css("display","none");
					var msg_full = "JPEGCam Flash Error: " + msg;
					$("form#patient_form_image_click").html(msg_full);
				}
				break;
			...
			case 'success':
				...
				var close_class=$(".popup").find(".close").length;
				if(close_class>0){
				   ...
				   $(".close").trigger("click");
					   ...
					   htmlStr2 += "Patients's image has updated successfully.";
					   htmlStr2 += "</span></div>";
