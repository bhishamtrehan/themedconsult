...
$(document).ready(function(){ 
    ...
    $('.filter-appnt-practitioners').hide();
		...
		var appntSettings;
		jQuery.ajax({
			url: baseUrl+'health_practitioner/appointment/fetch_settings',
			dataType:'json',
                        success:function(response)
			{
				...
				var mainHeight = ($(window).height()) - ($('.top-bar').height()) - ($('.widget-header').height()) - ($('.fc-header').height()) - (190);
                    ...
                    eventDrop: function(event, delta, revertFunc) {
						...
						popitup(baseUrl+'health_practitioner/appointment/edit_appointment?startDate='+dateTimestamp+'&startTime='+startHour+':'+startMinute+'&endTime='+endHour+':'+endMinute+'&endDate='+endTimestamp+'&resource='+event.resourceId+'&appointment_id='+event.id);
					...
					select: function(start, end, allDay, event, resourceId, view) {
                        ...
                        if(isOverlapping(toTimestamp(start),toTimestamp(end),calendar,resourceId) == false)
                        {
							if(view.name == 'resourceDay') {
								...
								popitup(baseUrl+'health_practitioner/appointment/add_appointment?startDate='+dateTimestamp+'&startTime='+startHour+':'+startMinute+'&endTime='+endHour+':'+endMinute+'&endDate='+endTimestamp+'&resource='+resourceId+patient_rebooking);
								calendar.fullCalendar('unselect');
							} else {
...
//                                               width = width / 4;
					...
					eventClick: function(calEvent, jsEvent, view) {
						...
						$.ajax({
							type: "POST",
							url: baseUrl+"health_practitioner/appointment/show_appointment_detail", 
								...
								$( ".btn-lg" ).trigger( "click" );
                                                                ...
                                                                $('#generate_invoice_link').click(function(){
                                                                 ...
                                                                 $('form#appoint_invoiceform').submit(function(e) {
                                                                     ...
                                                                     $.ajax({
                                                                             type: "POST",
                                                                             url: baseUrl+"health_practitioner/patients/generate_invoice",
                                                                             ...
                                                                             success: function(data){
                                                                                     ...
                                                                                     appoint_popitup(baseUrl+"health_practitioner/patients/open_invoice_info/"+patient_id+"/"+data+"/"+appoint_id)
                                                                             },
								...
								$('.cancel_appnt').click(function(){
									...
									var appntDate = $(this).attr('appointment_date');
         							jQuery.confirm({
								        ...
								        confirm: function(){
									        $('body').addClass("show_loader");
											$(window).attr('location', baseUrl+'health_practitioner/appointment/cancel_appointment?appnt_id='+appntId+'&appointment_date='+appntDate);
								        },
								        cancel: function(){
					...
					editable: true,
					resources: baseUrl+'health_practitioner/appointment/getLocPractitioners',
					events: baseUrl+'health_practitioner/appointment/get_calendar_appointments',
				});
...
$('#ID_for_cal_view').on('change', function() {
	...
	{
		window.location = baseUrl+'health_practitioner/dashboard';
	}
	else
	{
		window.location = baseUrl+'health_practitioner/rosterView';
	}
});